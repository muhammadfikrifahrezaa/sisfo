        ...
        <div class="col-lg-12">
            <div class="ibox ">
                <div class="ibox-title">
                    <h5><button class="btn btn-primary btn-sm" data-toggle="modal" data-mode="add" data-target="#ModalAddEdit"><i class="fa fa-plus-square mr-1"></i> Tambah Poli [F2]</button></h5>
                </div>
    ...
    $(document).ready(function() {
        ...
        $('#ModalAddEdit').on('shown.bs.modal', function(e) {
            ...
            if (button.data('mode') == 'edit') {
                ...
                $('#formAddEdit').attr('action', '{{ route("poli.store") }}/' + id).attr('method', 'PATCH')

                modal.find('#modal-title').text('Edit Poliklinik');
                modal.find('#name').val(closeTr.find('td:eq(1)').text())
                modal.find('#code').val(closeTr.find('td:eq(2)').text())

            } else {
                $('#formAddEdit').trigger('reset').attr('action', '{{ route("poli.store") }}').attr('method', 'POST')
                modal.find('#modal-title').text('Tambah Poliklinik');
            }
        })
